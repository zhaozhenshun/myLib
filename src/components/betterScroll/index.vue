      ...
      <div ref="listWrapper" class="scroll-list-wrapper">
        <slot>
          <ul class="scroll-list">
            <li class="scroll-item border-bottom-1px" v-for="(item, index) in data" :key="index" @click="clickItem(item)">{{item}}</li>
          </ul>
        </slot>
      </div>
      <slot name="pullup" :pullUpLoad="pullUpLoad" :isPullUpLoad="isPullUpLoad">
        <div class="pullup-wrapper" v-if="pullUpLoad">
          ...
          <div class="after-trigger" v-else>
            <spinner :size="20" color='rgb(252, 135, 0)' type="snake"></spinner>
          </div>
        </div>
      </slot>
    </div>
...
import { Spinner } from 'truck-mint-ui'
import Bubble from './lib/bubble.vue'
import { getRect, camelize } from './lib/until'
...
const SCROLL_EVENTS = [EVENT_SCROLL, EVENT_BEFORE_SCROLL_START, EVENT_SCROLL_END, EVENT_TOUCH_END]
...
export default {
  ...
  methods: {
    ...
    forcePullDownRefresh () {
      this.scroll && this.scroll.trigger('pullingDown')
    },
    refresh () {
      this._calculateMinHeight()
